...
@Component({
  selector: 'proyectos',
  templateUrl: './proyectos-i.component.html',
  styleUrls: ['./proyectos-i.component.scss']
})
export class ProyectosIComponent implements OnInit {
  proy:Proyecto[]; 
  dialogOpened=false;
  leg:number;
  data:any;
  ...
  ngOnInit()
  {
    ...
    {
     this.proy_serv.getProy_I().subscribe(
      proy=>
      {this.proy=proy}
      ,
  ...
  action(status) {
    if(status==='si')
    {
      this.proy_serv.descartar_proy(this.register.getUserLogged().usr.id_user,this.leg);
